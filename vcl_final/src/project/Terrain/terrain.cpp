...
void Terrain::init_terrain(){
    xy_z.resize(z_precision);
    for(int i=0;i<z_precision;i++)xy_z[i].resize(z_precision);
    ...
    for(size_t x=0;x<nb_points;x++){
        for(size_t y=0;y<=x;y++){
            ...
            float x0=terrain.position[x*(x+1)/2+y][0],y0=terrain.position[x*(x+1)/2+y][1],z0=terrain.position[x*(x+1)/2+y][2];
            xy_z[x_calc(x0)][y_calc(y0)]=z0;
        }
    }
for(size_t x=0;x<nb_points;x++){
    for(size_t y=0;y<=x;y++){
        float x0=terrain.position[x*(x+1)/2+y][0],y0=terrain.position[x*(x+1)/2+y][1],z0=terrain.position[x*(x+1)/2+y][2];
        std::cout<<z0<<" "<<evaluate_z2(x0,y0)<<std::endl;
    }
}
...
int Terrain::x_calc(float x0){
    return (int)((x0+2*l_ile)/(4*l_ile)*z_precision);
}
int Terrain::y_calc(float y0){
    return (int)((y0+2*l_ile)/(4*l_ile)*z_precision);
}
float Terrain::evaluate_z2(float x, float y){
    return xy_z[x_calc(x)][y_calc(y)];
}
