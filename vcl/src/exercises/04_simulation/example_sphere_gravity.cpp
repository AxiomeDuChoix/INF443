...
void scene_exercise::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    for(particle_structure& particle : particles)
    {
        ...
        v = v + dt*(F-0.003*v)/m;
        p = p + dt*v;
        // if (p.z < 0.0 && (p.x)*(p.x)+(p.y)*(p.y)<=4.0f){
        //     p = p+dt*v;
        // }
        // else{
        //     v = v + dt*(F-0.01*v)/m;
    ...
    // For those particles that have collided
    const float r = 0.05f; // radius of spheres
    const vec3 ez = {0,0,1};

    // Remove particles that are too low
    for(auto it = particles.begin(); it!=particles.end(); ++it)
        if((it->p.z < -3) && ((it->p.x)*(it->p.x)+(it->p.y)*(it->p.y)>4.0f))
            it = particles.erase(it);
        // else if (it->p.z < 0 && (it->p.x)*(it->p.x)+(it->p.y)*(it->p.y)<=4.0f){
        //     vec3 tg = (1/norm(it->v-dot(it->v,ez)*ez))* (it->v-dot(it->v,ez)*ez);
        //     it->v =dot(it->v,tg)*tg;
        // }
    ...
    // For those particles that have collided
    for(auto it = particles.begin(); it!=particles.end(); ++it)
        if((it->p.z-r <= 0) && ((it->p.x)*(it->p.x)+(it->p.y)*(it->p.y)<=4.0f)){
            vec3& v = it->v;
            vec3 tg = (1/norm(v-dot(v,ez)*ez))* (v-dot(v,ez)*ez);
            vec3 new_speed = dot(v,tg)*tg-dot(v,ez)*ez;
            v =new_speed;
