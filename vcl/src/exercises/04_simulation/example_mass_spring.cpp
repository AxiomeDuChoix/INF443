...
void scene_exercise::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    // Forces
    const vec3 f_spring  = spring_force(pA.p,pB.p, L0, K);
    const vec3 f_spring2  = spring_force(pB.p,pC.p, L0, K);
    ...
    const vec3 f_damping2 = -mu*pC.v; // TO DO: correct this force value
    const vec3 F2 = f_spring2+f_weight+f_damping2;

    // Numerical Integration (Verlet)
    {
        // Only particles B & C should be updated
    ...
    // particle pc
    sphere.uniform_parameter.translation = pC.p;
    sphere.uniform_parameter.color = {0,1,0};
    sphere.draw(shaders["mesh"], scene.camera);
