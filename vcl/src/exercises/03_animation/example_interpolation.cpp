...
static vec3 cardinal_spline_interpolation(float mu, float t, float t0, float t1, float t2, float t3, const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3);
...
void scene_exercise::frame_draw(std::map<std::string,GLuint>& shaders, scene_structure& scene, gui_structure& )
{
    ...
    const vec3 p = cardinal_spline_interpolation(mu,t,t0,t1,t2,t3,p0,p1,p2,p3);
...
static vec3 cardinal_spline_interpolation(float mu, float t, float t0, float t1, float t2, float t3, const vec3& p0, const vec3& p1, const vec3& p2, const vec3& p3){
    float s=(t-t1)/(t2-t1);
    float di1=mu*(t2-t1)/(t2-t0);
    float di2=mu*(t2-t1)/(t3-t1);
    float x1=(2*s*s*s-3*s*s+1)*p1[0]+(s*s*s-2*s*s+s)*di1*(p2[0]-p0[0])+(-2*s*s*s+3*s*s)*p2[0]+(s*s*s-s*s)*di2*(p3[0]-p1[0]);
    float x2=(2*s*s*s-3*s*s+1)*p1[1]+(s*s*s-2*s*s+s)*di1*(p2[1]-p0[1])+(-2*s*s*s+3*s*s)*p2[1]+(s*s*s-s*s)*di2*(p3[1]-p1[1]);
    float x3=(2*s*s*s-3*s*s+1)*p1[2]+(s*s*s-2*s*s+s)*di1*(p2[2]-p0[2])+(-2*s*s*s+3*s*s)*p2[2]+(s*s*s-s*s)*di2*(p3[2]-p1[2]);
    return {x1,x2,x3};
}
